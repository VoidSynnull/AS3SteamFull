package game.scenes.lands.shared.tileLib.generation.generators {
	...
	public class CaveGenerator extends MapGenerator {
		...
		public function CaveGenerator( tmap:TileMap ) {
		...
		private function makeCutBitmap( width:int, height:int, baseSize:int, randoms:WorldRandoms ):RandomMap {
			...
			bmd.perlinNoise( 1.5*baseSize, baseSize, 1, randoms.seed, false, false, 1, true, scrollArray );

			//overlay second perlin to reverse tunnel values and add variation
			var bmd2:BitmapData = new BitmapData( width, height, false, 0 );
			bmd2.perlinNoise( 4*baseSize, 4*baseSize, 1, randoms.seed + 1, false, true, 1, true, scrollArray );
			bmd.draw( bmd2, null, null, BlendMode.DIFFERENCE );
			
			//overlay third perlin to decrease cave frequency and add more shape variation
			bmd2.perlinNoise( 10*baseSize, 10*baseSize, 1, randoms.seed + 2, false, true, 1, true, scrollArray );
