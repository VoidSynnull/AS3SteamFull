package game.scenes.examples.customCharacter.states
{
	...
	public class JumpState extends CustomCharacterState
	{
		...
		{
			return (-node.motionTarget.targetDeltaY > node.characterMotionControl.inputDeadzoneY && Math.abs(-node.motionTarget.targetDeltaY / node.motionTarget.targetDeltaX) > SLOPE_RANGE)
		}
		...
		override public function start():void 
		{
			...
			if ( node.fsmControl.check(CharacterState.SWIM) )
			{
				...
				node.characterMotionControl.jumpDampener = node.characterMotionControl.jumpDampenerWater;
				//node.characterMotionControl.jumpDampener = node.waterCollider.viscosity;
				
				if( !node.waterCollider.surface )	// if not on surface, then is jumping underwater, apply velocity & switch to DIVE state
				{
					applyJumpVelocity( node, node.characterMotionControl.jumpDampener );	
			...
			{
				node.characterMotionControl.jumpDampener = 1;
			}
		...
		{	
			var characterMotionControl:CharacterMotionControl = node.characterMotionControl;
			...
			if(Math.abs(node.motionTarget.targetDeltaX) > _minXDistanceJumpSpin)
			{
				characterMotionControl.spinSpeed = characterMotionControl.spinJumpRotation;
				characterMotionControl.spinning = true;
				characterMotionControl.spinEnd = false;
				characterMotionControl.spinCount = 1;
			}
			
			applyJumpVelocity(node, characterMotionControl.jumpDampener);
		...
		protected function applyJumpVelocity( node:CustomCharacterNode, dampener:Number = 1 ):void
		{
			// cap factors
