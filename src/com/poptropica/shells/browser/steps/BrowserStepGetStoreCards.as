package com.poptropica.shells.browser.steps
{
	 ...
	 * @author rick hocker
	 */
	public class BrowserStepGetStoreCards extends ShellStep
	{
		public static var failedToGetCards:Boolean = true;
		// get store cards
		public function BrowserStepGetStoreCards()
		{
			super();
		...
		override protected function build():void
		{
			...
			{
				trace("BrowserShell :: BrowserStepGetStoreCards :: network NOT available, cannot retrieve store cards from server.");
				built();
			}
		}
		...
		{
			trace("BrowserShell :: BrowserStepGetStoreCards :: network IS available, getting store cards from server.");
				...
				for each (var card:Object in cardList)
				{
					...
					if (ItemManager.isStoreItemConverted(cardNum))
					{
						...
						if ((category != ItemManager.MEMBER_GIFT) && (card.pri != "0"))
						{
							...
							storeList.push(data);
							//trace("store card: " + cardNum + " " + card.name + " (" + category + ")");
						}
					}
				}
			}
