package org.assetloader.base
{
	...
	public class LoaderFactory
	{
		...
		protected var _platform : IPlatform;
		public function LoaderFactory(platform:IPlatform)
		{
			_platform = platform;
		}
		 ...
		 * @see org.assetloader.core.ILoader
		 */
		public function produce(id : String = null, type : String = "AUTO", request : URLRequest = null, params : Array = null) : ILoader
		{
			if(request)
			{
		...
		protected function constructLoader(type : String, id : String, request : URLRequest) : void
		{
			switch(type)
			{
				...
				case AssetType.SWF:
					//loads the correct loader per platform
					//IOS = IosSWFLoader
					//Web = SWFLoader
					//Android = AndroidSWFLoader
					if(request.url.search("http") != -1){
						_loader = new SWFLoader(request, id);
					}else{					
						_loader = this._platform.getInstance(ILoader,[request, id]) as AbstractLoader;
					}
					break;
				...
				case AssetType.GROUP:
					_loader = new AssetLoader(_platform,id);
					break;
