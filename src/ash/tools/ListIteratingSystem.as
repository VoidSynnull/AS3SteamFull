package ash.tools
{
	...
	public class ListIteratingSystem extends System
	{
		...
		protected var removeNodeClass : Boolean = true;
		...
		{
			nodeList = systemManager.getNodeList( nodeClass );
		...
		override public function removeFromEngine( systemManager : Engine ) : void
		{
			if(nodeList)
			{
				if( nodeAddedFunction != null && nodeList.nodeAdded)
				{
					nodeList.nodeAdded.remove( nodeAddedFunction );
				}
				if( nodeRemovedFunction != null && nodeList.nodeRemoved)
				{
					for( var node : Node = nodeList.head; node; node = node.next )
					{
			...
			if(removeNodeClass)
			{
				systemManager.releaseNodeList( nodeClass );
			}
		}
