package game.managers.stateMachine
{
	...
	public class StateMachine
	{
		...
		private var _states:Dictionary;	// Dict of State classes, with State's name variable as key
		private var _outMessage:StateMachineMessage;
		
		private var parentState:State;
		private var parentStates:Array;
		private var path:Array;
		
		public var messageSignal:Signal = new Signal( StateMachineMessage );
		...
		**/
		public function set initialState(stateName:String):void
		{
			if (_state == null && stateName in _states){
				...
				var _callbackEvent:StateMachineMessage = new StateMachineMessage(StateMachineMessage.ENTER_CALLBACK);
				...
				_outMessage = new StateMachineMessage(StateMachineMessage.TRANSITION_COMPLETE);
				_outMessage.toState = stateName;
				//dispatchEvent(_outMessage);
				messageSignal.dispatch(_outMessage);
			}
		}
		...
		**/
		public function changeState(stateTo:String):void
		{
			...
			if(!canChangeStateTo(stateTo))
			{
				...
				_outMessage = new StateMachineMessage(StateMachineMessage.TRANSITION_DENIED);
				_outMessage.fromState = _state;
				_outMessage.toState = stateTo;
				_outMessage.allowedStates = _states[stateTo].from;
				messageSignal.dispatch(_outMessage);
				return;
			}
			...
			if(path[0]>0)
			{
				var _exitCallbackEvent:StateMachineMessage = new StateMachineMessage(StateMachineMessage.EXIT_CALLBACK);
			...
			if(path[1]>0)
			{
				var _enterCallbackEvent:StateMachineMessage = new StateMachineMessage(StateMachineMessage.ENTER_CALLBACK);
			...
			_outMessage = new StateMachineMessage(StateMachineMessage.TRANSITION_COMPLETE);
			_outMessage.fromState = oldState ;
			_outMessage.toState = stateTo;
			messageSignal.dispatch(_outMessage);
		}
	}
