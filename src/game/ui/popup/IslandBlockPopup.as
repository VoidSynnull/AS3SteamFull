package game.ui.popup
{
	...
	public class IslandBlockPopup extends Popup
	{
		public function IslandBlockPopup( groupPrefix:String, container:DisplayObjectContainer=null)
		{
			super(container);
		...
		private function assetLoadingComplete():void
		{
			...
			if( false )// no longer split up this way
			{
				// setup buy button
		...
		{
			trace("IslandBlockPopup :: yutoob", _youTubePlayer.percentLoaded, "percent loaded");
		}
		...
		private function onMembershipBtn( entity:Entity ):void 
		{
			...
			var popup:MembershipPopup = HudPopBrowser.buyMembership(super.shellApi, _queryString);
			if(popup)
			{
				popup.removed.addOnce(removeBlocker);
			}
		}
		
		private function removeBlocker(...args):void
		{
		...
		{
			// return to map
			if(shellApi.sceneManager.currentScene is shellApi.sceneManager.gameData.mapClass)
			{
				super.close();
				return;
			}
			super.shellApi.loadScene( super.shellApi.sceneManager.gameData.mapClass );
		}
		...
		private function setupContent( xml:XML ):void
		{
			...
			this._queryString = DataUtils.useString(xml.queryString, "");
		...
		public function createText():void
		{
			...
			for (var i:int = 0; i < _textData.length; i++) 
			{
				...
				tf.height = ( !isNaN(textData.height) ) ? textData.height : _content.height;
				tf.width = ( !isNaN(textData.width) ) ? textData.width : _content.width * .9;
				...
				tf.x = ( !isNaN( textData.xPos ) ) ? textData.xPos : (_content.width - tf.width) * .5;
		...
		/** Query string added to membership url */
