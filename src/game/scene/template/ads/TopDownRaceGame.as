package game.scene.template.ads
{
	...
	import game.scenes.custom.TopDownRaceGameSystem;
	...
	/**
	 * Game template for top down race game.
	 ...
	 * Only one collision object can be on the screen at one time since we reuse the same object
	 ...
	 * @author uhockri
	 */
	public class TopDownRaceGame extends AdGameTemplate
	{
		...
		public var boostClip:MovieClip;						// reference to boost animation clip
		...
		public var obstacleData:Array = [];
		public var shooters:Array;
		// array of obstacles in scene
		...
		private var _raceSystem:TopDownRaceGameSystem;		// reference to TopDownRaceGameSystem
		...
		private var _animFrameRate:int = 8;					// frame rate for car timeline
		 ...
		 * Constructor 
		 */
		public function TopDownRaceGame()
		{
			this.id = "TopDownRaceGame";
		}
		...
		override public function setupGame(scene:QuestGame, xml:XML, hitContainer:DisplayObjectContainer):void
		{
			...
			backEntity1 = EntityUtils.createSpatialEntity(scene, _hitContainer["background1"]);
			backEntity2 = EntityUtils.createSpatialEntity(scene, _hitContainer["background2"]);
			if (_hitContainer["foreground1"] != null)
			{
				foreEntity1 = EntityUtils.createSpatialEntity(scene, _hitContainer["foreground1"]);
				foreEntity2 = EntityUtils.createSpatialEntity(scene, _hitContainer["foreground2"]);
			}
			startTileY = backEntity1.get(Spatial).y;
			
			// use XML now instead of grabbing from movie clip
			//tileHeight = backEntity1.get(Display).displayObject.height;
			...
			_raceSystem = TopDownRaceGameSystem(scene.addSystem( new TopDownRaceGameSystem(this), SystemPriorities.update ));				
		...
		override protected function parseXML(xml:XML):void
		{
			...
			// get frame rate for player timeline
		...
		{			
			...
			// convert clip if timeline
			if (clip.totalFrames != 1)
			{
				// determine frame rate and whether timeline should loop
				var frameRate:int = 32;
				var looping:Boolean = false;
				
				switch(type)
				{
					case RaceCollider.MOVING:
						// moving obstacles should use slower frame rate (for cars)
						frameRate = _animFrameRate;
						break;
					case RaceCollider.BOOST:
						// boost obstacles should loop
						looping = true;
						break;
				}
				
				var clipEntity:Entity = TimelineUtils.convertClip(clip, _scene, entity, roadEntity, looping, frameRate);
				// RLH: if clip has idle frame, then setup to loop on idle sequence
				if (clip.currentLabel == "idle")
				{
					// get timeline and play and setup signal for labels
		...
		override public function playerSelection(selection:int = 0):void
		{
			...
			else
			{
				...
				if (clip["boost"] != null)
				{
					boostClip = clip["boost"];
					boostClip.visible = false;			
				}
				...
				if (clip.totalFrames != 1)
					TimelineUtils.convertClip(clip, _scene, playerEntity, null, false, _animFrameRate);
		...
		public function gotHit(playerAtRight:Boolean, colliderType:String):void
		{
			...
			{
				// crash animation
				crashAnim(playerEntity, playerAtRight);
		 ...
		 */
		public function crashAnim(entity:Entity, away:Boolean):void
		{
		...
		private function endGame():void
		{
			...
			// remove system
			_scene.removeSystemByClass(TopDownRaceGameSystem);
